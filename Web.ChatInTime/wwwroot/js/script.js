...
function jcalendar_week(options){
	...
	//设置周日历
    var setdaydata = function (year, weeknum,reservationdatelist){
        ...
        var before_btn = opts.showbtn ? '<li><button class="switch_month beforem_btn">&lt;</button></li>' : '';
        var after_btn = opts.showbtn ? '<li><button class="switch_month afterm_btn">&gt;</button></li>' : '';
        ...
        for (var i = 0; i < 7; i++){
            ...
            if (reservationdatelist.length > 0) {
                for (var j = 0; j < reservationdatelist.length; j++) {
                    if (reservationdatelist[j] == date) {
                        week_day += '<li class="calendar_day' + dayaddclass + '" ' +
                            'date="' + date + '">' +
                            '<span class="calendar_day_i ' + addClass + '">' + newdate.getDate() + '</span>' +
                            '<div class="isShow1"><div class="isShowPoint1"></div></div >'+
                        '</li>';
                    } else {
                        count++;
                        if (count == reservationdatelist.length) {
                            week_day += '<li class="calendar_day' + dayaddclass + '" ' +
        ...
        console.log(before_btn + week_day + after_btn);
        //calendarid.find(".calendar_day_bar").html(week_day);
        calendarid.find(".calendar_day_bar").html(before_btn + week_day + after_btn);
	}
	...
	//DOM添加
    _this.confirmweek = function (year, weeknum, reservationdatelist){
		if(!year) year=$("#setyear").val();
		if(!weeknum) weeknum=$("#setweek").val();
		if(weeknum<1) weeknum=1;
		
        setdaydata(year, weeknum, reservationdatelist);
		...
		calendarid.find(".beforem_btn").off("click").on("click",function(){
            ...
            _this.confirmweek(beforeweekdata[0], beforeweekdata[1], reservationdatelist);
		})
		//下一周
		calendarid.find(".afterm_btn").off("click").on("click",function(){
            ...
            _this.confirmweek(afterweekdata[0], afterweekdata[1], reservationdatelist);
		})
		//day点击事件
	...
	_this.nowweek = function(){
        ...
        _this.confirmweek(weekdata[0], weekdata[1], reservationdatelist);
	}
	_this.nowweek();
}